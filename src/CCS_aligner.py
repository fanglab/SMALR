...
class mols_aligner:
	...
	"""
	def __init__( self, mols, orig_fastq, ref, nat_movie_name_ID_map, noAlign, chunk_id ):
		self.orig_fastq = orig_fastq
		self.ref        = ref
		self.chunk_id   = chunk_id

		sam_md, tmp_dir = self.align_CCS_mols_to_ref( mols.values(), nat_movie_name_ID_map, noAlign )
		self.parse_alignments( sam_md, mols, noAlign )
		# self.native_stats = self.get_native_read_stats( mols )
		shutil.rmtree(tmp_dir)

	def align_CCS_mols_to_ref( self, mols, nat_movie_name_ID_map, noAlign ):
		...
		def get_CCS_reads_for_aligning( mols, nat_movie_name_ID_map, noAlign ):
		...
		new_fastq, tmp_dir = get_CCS_reads_for_aligning( mols, nat_movie_name_ID_map, noAlign )
		sam_md = None
		if os.path.getsize(new_fastq) > 0:
			sam_md    		   = call_bwasw_samtools( new_fastq )
		return sam_md, tmp_dir

	def parse_alignments( self, sam_md, mols, noAlign ):
