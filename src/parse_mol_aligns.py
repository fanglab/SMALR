...
class molecule:
	...
	def load_entries( self, alignments ):
		...
		for subread_id, align in enumerate(alignments):
			...
			if self.mol_id == 102200 and strand == 1:
				debug = True
...
def read_alignments_file( alignments_flat_fn, chunk_id, contigName, prefix, lines_range, leftAnchor, rightAnchor ):
	"""
	...
	for i,line in enumerate(f.xreadlines()):
		...
		# We are going to load the molecule objects straight from here.
...
def generate_molecule_ZMW_map( mols, chunk_id ):
	...
	try:
		logging.debug("Chunk %s: removed %s double-loaded molecules (%.2f%%)." % (chunk_id,                  \
																			  len(multi_loaded_mol_ids), \
																			  100*len(multi_loaded_mol_ids)/float(len(mols.values()))))
	except ZeroDivisionError:
		logging.debug("Chunk %s: no molecules available for ZMW/moleculeID mapping!" % chunk_id)
...
class wga_molecules_processor:
	def __init__( self,               \
		...
		self.motifSitesPos_fn   = motifSites_pos
		self.motifSitesNeg_fn   = motifSites_neg
		self.leftAnchor         = leftAnchor
		self.rightAnchor        = rightAnchor
		self.wgaLibrary         = wgaLibrary
		self.split_mols         = split_mols

		if self.motifSitesPos_fn != None and self.motifSitesNeg_fn != None:
			logging.debug("Chunk %s: reading motif sites..." % self.chunk_id)
			self.sites_pos = read_in_motif_sites( self.motifSitesPos_fn  )
			self.sites_neg = read_in_motif_sites( self.motifSitesNeg_fn  )
	...
	def __call__( self ):
		...
		if self.wgaLibrary=="long":
			lines_chunk_size = 200
		elif self.wgaLibrary=="short":
		...
		while keep_going:
			...
			logging.debug("Chunk %s: reading alignments..." % self.chunk_id)
			self.mols = read_alignments_file( self.alignments_flat_fn, self.chunk_id, self.contigName, self.prefix, lines_range, self.leftAnchor, self.rightAnchor)
			...
			logging.debug("Chunk %s (subchunk %s): loaded %s molecules so far." % (self.chunk_id, i, nMols_loaded))
			i            += 1
			
			pct_through_file = float(min(end, nlines)) / nlines * 100
			logging.info("Chunk %s: %.1f%% of WGA alignments loaded (%s molecules)." % (self.chunk_id, pct_through_file, nMols_loaded))
	...
	def create_agg_IPD_arrays( self ):
		...
		self.ipdArrays = {0:{}, 1:{}}
		# self.ipdArray_subread_map = {}
		for mol in self.mols.values():
			for entry in mol.entries.values():
				# position_in_array = len(self.ipdArrays[entry.strand][entry.pos])
				tmp_ipdArrays[entry.strand][entry.pos].append(entry.ipd)
				# self.ipdArray_subread_map[ (mol.mol_id, entry.strand, entry.pos, position_in_array) ] = entry.subread
...
class native_molecules_processor:
	...
	def __call__( self ):
		nlines               = alignment_file_len(self.alignments_flat_fn)
		if self.nativeLibrary=="long":
			lines_chunk_size = 200
		elif self.nativeLibrary=="short":
			lines_chunk_size = 3000
		start                = 0
		end                  = start + lines_chunk_size
		lines_range          = [start, end]
		...
		self.chunk_output_fn = "test.%s.out" % self.chunk_id
		self.var_chunk_fn    = "vars.%s.tmp" % self.chunk_id
		if self.noAlign:
			pass
		else:
		...
		while keep_going:
			...
			if len(self.mols.values()) > 0:
				...
				if self.nativeLibrary=="short":
					...
					logging.debug("Chunk %s (subchunk %s): deleting %s molecules due to too many positions with low coverage." % (self.chunk_id, \
																																  i, \
																																  len(del_me)))
			...
			if len(self.mols.values()) > 0:
				...
				logging.debug("Chunk %s (subchunk %s): deleted %s (%.1f%%) off-limits positions." % (self.chunk_id, \
																									 i, \
																									 tot_entries_deleted, \
																									 pct_deleted))

				# Generate the IPD arrays per genomic position/strand by aggregating all IPD entries across molecules (self.ipdArrays)
				logging.debug("Chunk %s: generating IPD arrays..." % self.chunk_id)
				for mol in self.mols.values():
				...
				logging.debug("Chunk %s: running comparisons..." % self.chunk_id)
				for mol in self.mols.values():
					self.get_scores( mol )

				mols_w_results = len([mol for mol in self.mols.values() if len(mol.output)>0])
				logging.debug("Chunk %s (subchunk %s): %s molecules generated comparison test output" % (self.chunk_id, i, mols_w_results))
				...
				logging.debug("Chunk %s (subchunk %s): loaded and analyzed %s molecules so far." % (self.chunk_id, i, nMols_loaded))

				pct_through_file = float(min(end, nlines)) / nlines * 100
				logging.info("Chunk %s: %.1f%% of native alignments loaded. %s molecules analyzed." % (self.chunk_id, pct_through_file, nMols_loaded))
	...
	def apply_per_mol_coverage_filter( self ):
		...
		"""
		logging.debug("Chunk %s: %s molecules prior to coverage filtering." % (self.chunk_id, len(self.mols.values())))
		...
		for mol_id, mol in self.mols.iteritems():
			...
			if mol.cov >= self.nativeCovThresh or self.nativeLibrary=="long":
				mols_dict_covFiltered[mol_id] = mol

		if len(mols_dict_covFiltered.keys())==0:
			logging.debug("Chunk %s: no native molecules made it through the coverage filter!" % self.chunk_id)
		self.mols = mols_dict_covFiltered
		logging.debug("Chunk %s: %s molecules have coverage >= %.1f" % (self.chunk_id, len(self.mols), self.nativeCovThresh))
	...
	def create_arrays( self, mol ):
		...
		for strand in ipdArrays.keys():
			for pos in ipdArrays[strand].keys():
				if len(ipdArrays[strand][pos]) >= self.nativeCovThresh or self.nativeLibrary=="long":
			...
			if len(mol.ipdArrays[strand].keys()) > 0:
				for pos, array in mol.ipdArrays[strand].iteritems():
					if strand in set(self.control_ipds.keys()) and pos in set(self.control_ipds[strand].keys()):
						...
						logging.debug("No entry for position %s in the localized control dictionary (+/-2000bp from a sampled position in the native molecule)" % pos)
